...
[![SVcustos — SV(49,7) · Documento 6 de 8](assets/thumbnail_Github_n49.png)](https://github.com/juantoniolloretegea/SVcustos-dataset/raw/main/assets/SVcustos_Doc6_n49_video.mp4)
...
| Doc 5 | 9 | 3×3 | 19.683 | n₁ ≥ 7 | 163 |
| Doc 5 | 36+9 | par | ~2,95×10²¹ | max(cls₃₆, cls₉) | ver `pairs/` |
| Doc 6 | 49 | 7×7 | ~2,39×10²³ | n₁ ≥ 38 | ~6,93×10¹³ |

El umbral de cada nivel coincide con el campo `threshold_intrusion` de los ficheros de configuración `config/nXX.yaml`. La fila del par corresponde al Documento 5, cuya configuración está en `config/pair_n36_n9.yaml`. La fila de n=49 corresponde al Documento 6 (previsto).
...
Ejemplos análogos para n=9, n=25, n=36 y n=49 (Doc 5, Doc 3, Doc 4 y Doc 6):

```bash
python generate_dataset.py --level n9
python generate_dataset.py --level n25
python generate_dataset.py --level n36
python generate_dataset.py --level n49
```

Por defecto se crean 3.000 imágenes (1.000 por clase) en `data/nXX/` con estructura tipo ImageFolder. Para n=16, la estructura es:
...
Para generar el dataset del par (Doc 5):
...
(Análogo para n=9, n=25, n=36 y n=49 cambiando `--level` y el nombre del modelo.)
...
Este mismo valor se fija en el campo `threshold_intrusion` de cada fichero `config/nXX.yaml` para garantizar coherencia entre la teoría y la generación automática de etiquetas.
...
│
├── config/
│   ├── n9.yaml               ← Configuración Doc 5 (n=9, célula de integridad)
│   ├── n16.yaml              ← Configuración Doc 2 (n=16)
│   ├── n25.yaml              ← Configuración Doc 3 (n=25)
│   ├── n36.yaml              ← Configuración Doc 4 (n=36)
│   ├── n49.yaml              ← Configuración Doc 6 (n=49, previsto)
│   └── pair_n36_n9.yaml      ← Configuración del par (Doc 5)
│
├── pairs/                    ← Subproyecto: células SV en par (Doc 5)
│   ├── README.md             ← Documentación del subproyecto
│   └── generate_pair_dataset.py  ← Generación del dataset del par
│
...
No hace falta modificar ningún script: el número de radios del polígono se deriva siempre de la longitud del vector ternario.

## Subproyecto: células SV en par (Documento 5)

El Documento 5 de la serie introduce las **células SV en par**
como mecanismo para combinar dos células simples ya entrenadas.

En este repositorio, ese diseño se concreta en el subproyecto
`pairs/`, cuyo primer caso implementado es el par:

- **SV(36,6)** – célula principal (n = 36 = 6²),
- **SV(9,3)** – célula de integridad (n = 9 = 3²),
- **SV(par)** – resultado del par con la regla de severidad:

> INTRUSIÓN > INDETERMINADO > NORMAL,
> SV(par) = max(SV(36,6), SV(9,3)).

El fichero `config/pair_n36_n9.yaml` describe esta configuración y
el script `pairs/generate_pair_dataset.py` genera el dataset del par.

Este módulo **no redefine** una nueva célula de 45 parámetros, sino que
modela la decisión conjunta a partir de las salidas de las células
n=36 y n=9, tal como se describe en el Documento 5.

## Alcance del repositorio y relación con SVperitus

Este repositorio está dedicado al sistema **SVcustos** y a sus extensiones
directas: niveles n = 9, 16, 25, 36, 49 y el par SV(36,6)+SV(9,3) del Documento 5.
